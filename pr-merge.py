...
armin_reviewer = next((reviewer for reviewer in reviewers_data["reviews"] if reviewer['user']['login'] == 'armin-mahina'), None)
...
if "statuses" not in statuses_data:
    # Create the status check if it does not exist yet
    status_payload = {
        "state": "pending",
